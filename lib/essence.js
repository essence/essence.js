...
Essence.prototype.extract = function(url, options, callback) {
	...
	var extract = co(function *() {
		var providers = this._providersFor(url);
		var error;
		...
		throw error || new Error('Unable to find a suitable provider');
	}).bind(this);
...
/**
 *	Yields providers capable of handling the given URL.
 *
 *	@param {string} url URL.
 *	@yields {Provider} Provider.
 */
Essence.prototype._providersFor = function *(url) {
	for (var i in this._config) {
		var config = this._config[i];

		if (config.scheme.test(url)) {
			yield this._provider(config);
		}
	}
};
